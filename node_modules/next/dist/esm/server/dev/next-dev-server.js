...
export default class DevServer extends Server {
    ...
    async invokeIpcMethod(method, args) {
        ...
        if (ipcPort) {
            const res = await invokeRequest(`http://${this.hostname}:${ipcPort}?method=${method}&args=${encodeURIComponent(JSON.stringify(args))}`, {
                method: "GET",
                headers: {}
            });
            const chunks = [];
    ...
    async runInstrumentationHookIfAvailable() {
        if (this.actualInstrumentationHookFile) {
            ...
            try {
                const instrumentationHook = await require(pathJoin(this.distDir, "server", "instrumentation"));
