...
export class NextTypesPlugin {
    ...
    apply(compiler) {
        ...
        const handleModule = async (mod, assets)=>{
            ...
            const relativeImportPath = path.join(this.getRelativePathFromAppTypesDir(relativePathToApp)).replace(/\.(js|jsx|ts|tsx|mjs)$/, "").replace(/\\/g, "/");
        ...
        compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation)=>{
            compilation.hooks.processAssets.tapAsync({
                ...
                if (this.typedRoutes) {
                    ...
                    const packageJsonAssetPath = assetDirRelative + "/" + normalizePathSep(packageJsonTypePath);
                    assets[packageJsonAssetPath] = new sources.RawSource('{"type": "module"}');
                    const linkTypePath = path.join("types", "link.d.ts");
                    const linkAssetPath = assetDirRelative + "/" + normalizePathSep(linkTypePath);
