...
const nextDev = async (argv)=>{
    ...
    if (args["--turbo"]) {
        var _rawNextConfig_experimental;
        isTurboSession = true;
        ...
        const runDevServer = async ()=>{
            ...
            try {
                ...
                const setupFork = async (newDir)=>{
                    ...
                    if (((_config_experimental = config.experimental) == null ? void 0 : _config_experimental.appDir) && !devServerOptions.useWorkers) {
                        _log.error(`Disabling dev workers is not supported with appDir enabled`);
                    }
                };
                cleanupFns.push(()=>{
