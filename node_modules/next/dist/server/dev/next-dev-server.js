...
const _serveripc = require("../lib/server-ipc");
...
class DevServer extends _nextserver.default {
    ...
    setupWebSocketHandler(server, _req) {
        if (!this.addedUpgradeListener) {
            var _req_originalRequest_socket;
            this.addedUpgradeListener = true;
            server = server || ((_req_originalRequest_socket = _req == null ? void 0 : _req.originalRequest.socket) == null ? void 0 : _req_originalRequest_socket.server);
            if (!server) {
    ...
    async invokeIpcMethod(method, args) {
        ...
        if (ipcPort) {
            const res = await (0, _serveripc.invokeRequest)(`http://${this.hostname}:${ipcPort}?method=${method}&args=${encodeURIComponent(JSON.stringify(args))}`, {
                method: "GET",
                headers: {}
            });
            const chunks = [];
    ...
    async runInstrumentationHookIfAvailable() {
        if (this.actualInstrumentationHookFile) {
            ...
            try {
                const instrumentationHook = await require((0, _path.join)(this.distDir, "server", "instrumentation"));
